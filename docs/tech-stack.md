...
- **Rust**，cargo workspace，工具链锁定（`rust-toolchain.toml`，当前 1.95.0）。
...
`crates/` 都是**组件库**（细分、可复用）；`bots/` 是我的 bot；`apps/` 是命令行 + TUI 应用；全功能 GUI 由 Swift 另做。
...
**全功能 GUI = 独立 Swift 应用**（macOS 原生，SwiftUI / AppKit），不在本 workspace；经 `ffi` 链接引擎或驱动 `cli`。Rust 端不做正式 GUI。
...
- **纯组件细分、可复用**：`model / rules / opening / record / protocol` 无 I/O、无全局状态——便于本项目、其它工具、将来 **Swift GUI 经 FFI** 复用。`bot` 因含子进程 / 线程不算纯组件，但仍是单一 crate。
...
- **GUI 由 Swift 另做、不急**：Rust 端做引擎 + cli / tui + 将来的 `ffi` 边界。
- **配置用文件**：跨平台、可版本化、易测试。
